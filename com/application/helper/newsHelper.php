...
class newsHelper {
	...
	function pastEvent($all=false,$list=false, $id=false, $gallery=false){
		...
		// pr($qData);
			if($qData) {
				...
				if ($gallery){
					...
					foreach ($qData as $key => $val){
						$qData[$key]['gallery'] = $this->getGalleryEvent($id);
					}
	...
	function getGalleryEvent($cid=false)
	{
		...
		$sql = "SELECT * FROM {$this->dbshema}_news_content_repo WHERE otherid = {$cid} AND n_status = 2";
		// pr($sql);
		$res = $this->apps->fetch($sql,1);
