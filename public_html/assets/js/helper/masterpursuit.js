	...
	*/
	...
	function readMessage(data)
	{
		...
		// data.rec.id,data.rec.image_profile,subject,content,
		var html = "";
		...
		html += "			<h1>"+data[2]+"</h1>";
		if (data[4]){
		html += "			<h4>Sender : "+data[4]+"</h4><br />";
		}else{
		html += "			<h4>Sender : MARLBORO</h4><br />";
		}
...
//		html += "	<thead>";
	//	html += "		<tr>";
		...
		html += "	<tbody>";
		html += "		<tr>";
		html += "			<td><input type='checkbox' name='' value='' id='selectallinbox'></td>";
		html += "			<td colspan='2'><div class='page-arrow fr'><a href='javascript:void(0)' class='prev-message-page prevInbox '>&nbsp;</a><a href='javascript:void(0)' class='next-message-page nextInbox'>&nbsp;</a></div><a class='fr deleteInbox deletetAllMessage' href='javascript:void(0)'>Delete</a></td>";
		html += "		</tr>";
		$.each(rec, function(i,e){
			...
			html += "				<form action=''>";
			if (e.type==1){html += "					<input type='checkbox' name='' value='"+e.id+"' prop='"+e.type+"' class='selectedId'> ";}
			
			var substr = e.message;			
			
			html += "				</form>";
			html += "			</td>";
			html += "			<td><span class='messagepreview' prop='"+e.fromid+"' content='"+e.message+"' from='"+e.name+"' replay='"+e.type+"' idmesg='"+e.id+"' subject='"+e.subject+"'>"+substr.substring(0,30)+"....</span></td>";
	...
	$(document).on("click","#inboxcounting",function(){
		...
		var emailid = $(this).attr('hiddenid');
		...
		var inboxNotif = parseInt($(this).attr('inbox'));
	...
	$(document).on('click', '.messagepreview', function() {
		...
		$.post(basedomain+'pursuit/readmessage',{id:id, type:type,from:getid},function(data){
			
			if (data.status==true){
				// console.log(data);
				
				var myMessage=[data.rec.id,data.rec.image_profile,subject,content, data.rec.name, replay];
	...
	$(document).on('blur','#recepienttxt',function(){
		$('.taggingusersoninbox').css('display','none');
	...
	function comoposeMessage()
	{
        ...
        html += "                            </div>";
        html += "                            <div class='row'>";
        html += "                            	<a class='fr landbtn landbtn2 sendMessage' href='javascript:void(0)'>Send</a>";
        html += "                            </div>";
        html += "                        </div>";
